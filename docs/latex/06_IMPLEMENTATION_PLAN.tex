...
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{\input{figures/gantt}}
...
\hypertarget{pilot}{%
\section{Pilot: one recording before Phase 0 (done)}\label{pilot}}
\kctchanged{1.2}

Before building the full system we measured one real recording, NRCCW\_KSM09 (2~h~05~min, 1\,237 turns), to learn how well its transcript aligns and how stock Whisper does. The pilot lives in \texttt{src/\allowbreak{}transcribe/\allowbreak{}pilot/} and becomes the alignment and evaluation core of Phases 1 and 3.
...
\bottomrule\noalign{}
\endlastfoot
\texttt{pilot.run} & DOCX + audio → alignment (TRD §4.3), 2--30~s segments with flags, zero-shot Whisper (WER, CER, per-language WER, switch-point errors, words in untranscribed gaps), \texttt{report.md/json}, optional review chunks \\
\texttt{pilot.batch} & Runs every audio/DOCX pair in a folder and writes \texttt{summary.md} \\
Kaggle runbook & \texttt{scripts/kaggle\_pilot.sh} + \texttt{notebooks/kaggle\_pilot.ipynb}; see \href{07_KAGGLE_RUNBOOK.pdf}{07 Kaggle Runbook} \\
\texttt{pilot.make\_mock} & Known-answer smoke test from public FLEURS clips \\
\texttt{pilot.review\_stats} & Review speed from the review app's save timestamps \\
Tests & 24 tests in \texttt{tests/pilot} \\
\end{longtable}

\textbf{Results on NRCCW\_KSM09 (alignment):} words with timings 11\,590 of 12\,305; 1.70~h of segments, 0.91~h clean; no word more than 30~s from a transcriber timestamp; held-out median error 1.3~s. The laptop CPU computes emissions at about 0.8$\times$ real time and runs Whisper small at about 2$\times$ real time, so heavy runs move to Kaggle.

\textbf{Also done early:} P0.3 (Transcribe no longer overwrites reviewer text) and the \texttt{R:} speaker-label fix.

\textbf{Still open:} zero-shot Whisper numbers on correctly aligned segments (the first run used the old alignment), and the same pilot on the other 10 recordings. Both come from one Kaggle run.
...
P1.3 & \texttt{corpus/\allowbreak{}align.\allowbreak{}py}: an \texttt{Aligner} protocol wrapping the pilot's two-pass aligner (\texttt{pilot/\allowbreak{}ctc\_\allowbreak{}align.\allowbreak{}py}: relative confidence, anchors, block fill bounded by transcript timestamps) & Pilot numbers hold on every recording in the batch summary: $\ge$ 90\% of words timed, no marker error over 30~s \\
...
P1.7 & \textbf{Calibrate thresholds} \emph{(done in the pilot for NRCCW\_KSM09)}: check the anchor and \texttt{low\_\allowbreak{}alignment} thresholds against transcriber timestamps on each new recording & Marker check in every \texttt{report.md}; held-out median error $\le$ 3~s \\
\end{longtable}
...
\textbf{Speaker grouping (before choosing test/dev):} a one-off script, \texttt{corpus/speaker\_groups.py}, embeds each recording's speakers (ECAPA-TDNN, CPU) and lists cross-recording pairs with high similarity. You confirm them, combine them with the naming convention, and the groups are stored so the dataset builder never splits a group.

\textbf{Labelling track (parallel, starts at the end of P2):} review the current 11 recordings (8 h 56 min, ≈ 13--14 h of review at 1.5$\times$ real time): \textbf{test\_v1 = 2 recordings (≈ 1.5 h)} and \textbf{dev\_v1 = 1 recording (≈ 50 min)} first, then \textbf{train = 8 recordings (≈ 6.5 h)}. The long-run targets (test ≈ 3 h / ≥ 15 recordings, train ≥ 20 h) are reached as new audio arrives. Review the test/dev recordings first; they matter most and must be the most carefully checked.
...
\tightlist
\item
  You: run the pilot on Kaggle for all recordings (\href{07_KAGGLE_RUNBOOK.pdf}{07 Kaggle Runbook}) and send \texttt{summary.md} plus each \texttt{report.md}.
\item
  Together: read the zero-shot Whisper numbers and the per-recording alignment coverage; decide which recordings are usable as they are.
\item
  You: send the recording names so we can form speaker groups, then choose \texttt{test\_\allowbreak{}v1} (2 groups, about 1.5~h) and \texttt{dev\_\allowbreak{}v1} (1 group, about 50~min).
\item
  Me: Phase 0 on this branch, carrying the pilot code into the package layout.
\end{enumerate}

\kctchangelog
\begin{kctversions}
1.0 & 2026-09-24 00:36 & \texttt{053e09d} & First LaTeX version (converted from the Markdown draft of 2026-09-23). \\
1.1 & 2026-09-24 00:45 & \texttt{695b407} & Labelling track sized for the real corpus (11 recordings, 8~h~56~min). \\
1.1 & 2026-09-24 01:53 & \texttt{8a3be55} & Speaker grouping step before choosing test/dev. \\
1.2 & 2026-09-24 22:26 & this commit & New Pilot section with results; P1.3 and P1.7 updated; new immediate next steps. \\
\end{kctversions}

\kctchange{1.2}{Pilot: one recording before Phase 0 (new section)}{pilot}
 {none (new section) \textperiodcentered\ \href{archive/v1.1/06_IMPLEMENTATION_PLAN.pdf}{v1.1 PDF}}
 {No pilot; the plan went straight from Foundations to the alignment pipeline.}
 {Records what was built for the single-recording pilot, the NRCCW\_KSM09 alignment results, the tasks finished early (P0.3) and what is still open.}

\kctchange{1.2}{Phase 1, tasks P1.3 and P1.7}{phase-1-alignment-pipeline-7-days}
 {\kctsrc{8a3be55a33b1a843a27d87795a74872523d940bb}{06_IMPLEMENTATION_PLAN.tex}{57}{61}{v1.1 source, lines 57--61} \textperiodcentered\ \href{archive/v1.1/06_IMPLEMENTATION_PLAN.pdf}{v1.1 PDF}}
 {P1.3: ``an MMS implementation (windowed emissions, turn-anchored alignment, per-word scores)''; acceptance ``mean word boundary error < 100 ms vs. hand labels''. P1.7: ``Calibrate θ: align one real recording, hand-check 50 segments''.}
 {P1.3 wraps the pilot's two-pass aligner; acceptance is the pilot numbers holding on every recording. P1.7 is done for NRCCW\_KSM09 using transcriber timestamps instead of hand-checked segments, and repeats per recording through the report's marker check.}

\kctchange{1.2}{Immediate next steps}{immediate-next-steps}
 {\kctsrc{8a3be55a33b1a843a27d87795a74872523d940bb}{06_IMPLEMENTATION_PLAN.tex}{215}{226}{v1.1 source, lines 215--226} \textperiodcentered\ \href{archive/v1.1/06_IMPLEMENTATION_PLAN.pdf}{v1.1 PDF}}
 {1. Confirm decisions. 2. Start Phase 0 (P0.1--P0.5). 3. Pick about 10\% of recordings for \texttt{test\_v1}.}
 {1. Kaggle pilot run on all recordings. 2. Review the numbers together. 3. Speaker groups, then test/dev. 4. Phase 0.}

\kctchange{1.1}{Phase 2: labelling track and speaker grouping}{phase-2-review-ui-v2-6-days}
 {\kctsrc{053e09d4042e52bc8dfa98423c13d99a42a60109}{06_IMPLEMENTATION_PLAN.tex}{85}{85}{v1.0 source, line 85} \textperiodcentered\ \href{archive/v1.0/06_IMPLEMENTATION_PLAN.pdf}{v1.0 PDF}}
 {``review enough recordings to build test\_v1 ≈ 3 h (≥ 15 recordings, stratified), dev\_v1 ≈ 2--3 h (≥ 10 recordings), and train ≥ 20 h for v1.''}
 {Review the current 11 recordings (8~h~56~min): test\_v1 = 2 recordings (≈~1.5~h), dev\_v1 = 1 (≈~50~min), train = 8 (≈~6.5~h); long-run targets reached as audio arrives. New \emph{speaker grouping} step (\texttt{corpus/speaker\_groups.py}) before choosing test/dev.}
\end{document}
